...
export const Route = createFileRoute('/library/collections/$id')({
...
function CollectionDetailPage() {
	...
	const [title, setTitle] = useState('合集')
	const [videos, setVideos] = useState<SearchHit[] | null>(null)

	useEffect(() => {
		let cancelled = false
		void trpcClient.bili.collection
			.query({ id })
			.then((result) => {
				if (cancelled) return
		...
		return (
			<Empty>
				<EmptyHeader>
					...
					<EmptyDescription>正在读取合集。</EmptyDescription>
				</EmptyHeader>
			</Empty>
		)
	}
